...
# מנסה כמה sport keys כי שם המונדיאל משתנה לפי עונה
SPORT_KEYS = [
...
PREFERRED_BOOKMAKERS = ["pinnacle", "bet365", "williamhill", "unibet", "bwin", "draftkings", "fanduel"]
...
def _get(endpoint: str, params: dict) -> list | dict | None:
    if not ODDS_API_KEY:
        print("[OddsAPI] ❌ ODDS_API_KEY חסר")
        return None
    ...
    try:
        ...
        print(f"[OddsAPI] קריאות שנותרו: {remaining}")
        if res.status_code in (401, 403):
            print("[OddsAPI] ❌ API Key לא תקין")
            return None
        if res.status_code == 422:
            return None  # sport key לא קיים — נמשיך לבא
        if res.status_code == 429:
            print("[OddsAPI] ❌ חרגת ממכסה")
            return None
...
def _get_events() -> list:
    """מנסה כל sport key עד שמוצא משחקים."""
    for sport_key in SPORT_KEYS:
        ...
        if data and len(data) > 0:
            print(f"[OddsAPI] ✅ {sport_key} (EU): {len(data)} משחקים")
            return data
        ...
        if data and len(data) > 0:
            print(f"[OddsAPI] ✅ {sport_key} (US): {len(data)} משחקים")
            return data

    print("[OddsAPI] ⚠️ לא נמצאו משחקים בשום sport key")
    return []
...
def list_available_matches() -> list[str]:
    ...
    data = _get_events()
    return [f"{e.get('home_team')} vs {e.get('away_team')} ({e.get('commence_time','')[:10]})" for e in data]
...
def get_live_odds(home_team: str, away_team: str) -> dict | None:
    ...
    if not event:
        ...
        print(f"[OddsAPI] זמינים: {available}")
        return None
